...
| Area | Files | What |
|---|---|---|
| Types | `constants/type_constants.asm`, `data/types/names.asm`, `data/types/type_matchups.asm`, `engine/battle/core.asm` | `WONDER` ($09, the Ghost variant — renamed from the video's GHOSTT), `GUARD` ($0A, a copy of Bug's **defensive** matchups) and `STEEL` ($0B), all in the physical range; Shedinja is WONDER/GUARD so the status screen reads `TYPE1/ WONDER`, `TYPE2/ GUARD`. **WONDER and GUARD are defensive-only: they exist on exactly one Pokemon and on no move**, so they never appear in the attacker column of the type chart. Wonder Guard is the block of `x, WONDER, NO_EFFECT` rows. **Those rows must stay last in the chart**: the engine multiplies damage by every matching row but only remembers the *last* row's multiplier for the message, so with `POISON→GUARD ×2` after `POISON→WONDER ×0` Poison Sting printed "attack missed" instead of "doesn't affect". STAB (`AdjustDamageForMoveType`) canonicalizes the *attacker's* types only — WONDER counts as GHOST and GUARD as BUG — so Shedinja keeps STAB on ordinary Ghost and Bug moves.
| Moves | `constants/move_constants.asm`, `data/moves/{names,moves,animations,sfx}.asm`, `data/battle/critical_hit_moves.asm` | Shadow Claw / Shadow Sneak / Shadow Ball (plain **GHOST**) / Metal Claw (STEEL) inserted **before** STRUGGLE (Metronome asserts `NUM_ATTACKS == STRUGGLE`). Animations reuse Slash / Quick Attack / Night Shade / Scratch. |
...
- STAB probe (`wDamageMultipliers` bit 7): Lick (Ghost), Leech Life (Bug) and Shadow Claw (Ghost) all get STAB on the WONDER/GUARD Shedinja; Scratch (Normal) and Metal Claw (Steel) don't.
- Shadow Claw effectiveness (enemy restricted to Growl so Shedinja always attacks): Rattata and Spearow **immune** (Ghost can't hit Normal), Drowzee and Abra **damaged** ×1, Gastly **damaged** ×2, Geodude ×1.
...
**Deliberate vanilla change:** the Gen 1 `GHOST → PSYCHIC = no effect` bug was removed from the type chart. Once the Shadow moves became real GHOST moves they inherited it, which would have made Shedinja's only good STAB useless against Sabrina's gym — contradicting decision #7 (Part C), which was to let these moves hit Psychics normally. The side effect is that vanilla Ghost moves (only the Gastly line's Lick and Night Shade) can now hit Psychic types too. To restore the bug instead, put `db GHOST, PSYCHIC_TYPE, NO_EFFECT` back in `data/types/type_matchups.asm`.

Another consequence of a defensive-only type: Gen 1 Conversion copies the target's types to the user, so a Porygon using it on Shedinja would gain WONDER/GUARD — and with them Wonder Guard. Transform does the same. Both are unreachable in a solo run and are left as-is.
